...
Authors: Chris Hughes, Johannes H�lzl, Kim Morrison, Jens Wagemaker
-/
-- module
...
variable {R : Type u} {S : Type v} {T : Type w} {? : Type y} {a b : R} {m n : ?}
...
-- @[simp] ? attach the attribute at the moment of definition.
-- attribute [simp] foo ? attach it afterwards, to an already-defined lemma/def.


/-- `eval? f x p` evaluates a polynomial `p : R[X]` with coefficients mapped along
the ring hom `f : R ?+* S` and variable sent to `x : S`, i.e.
$\mathrm{eval}_2(f,x,p) = \sum_e f(a_e)\,x^e$ for $p = \sum_e a_e X^e$. -/
@[aesop norm simp]
irreducible_def eval? (p : R[X]) : S :=
  p.sum fun e a => f a * x ^ e


/-- `eval?_eq_sum` states that `eval? f x p` is implemented by summing over all
terms of `p`, multiplying each mapped coefficient by the appropriate power of `x`;
intuitively, evaluation is just �add up all the terms `coefficient * x^power`�. -/
theorem eval?_eq_sum {f : R ?+* S} {x : S} : p.eval? f x = p.sum fun e a => f a * x ^ e := by
  rw [eval?_def]
  --aesop-- if we use the attribute annotation before
  --aesop (add norm eval?_def)


theorem eval?_congr {R S : Type*} [Semiring R] [Semiring S] {f g : R ?+* S} {s t : S}
    {? ? : R[X]} : f = g ? s = t ? ? = ? ? eval? f s ? = eval? g t ? := by
...
theorem eval?_finset_sum (s : Finset ?) (g : ? ? R[X]) (x : S) :
    (? i ? s, g i).eval? f x = ? i ? s, (g i).eval? f x :=
  map_sum (eval?AddMonoidHom f x) _ _

theorem eval?_ofFinsupp {f : R ?+* S} {x : S} {p : R[?]} :
...
theorem eval_finset_sum (s : Finset ?) (g : ? ? R[X]) (x : R) :
...
theorem comp_assoc {R : Type*} [CommSemiring R] (? ? ? : R[X]) :
    (?.comp ?).comp ? = ?.comp (?.comp ?) := by
  refine Polynomial.induction_on ? ?_ ?_ ?_ <;>
    � intros
      simp_all only [add_comp, mul_comp, C_comp, X_comp, pow_succ, ? mul_assoc]

@[simp] lemma sum_comp (s : Finset ?) (p : ? ? R[X]) (q : R[X]) :
...
protected theorem map_sum {? : Type*} (g : ? ? R[X]) (s : Finset ?) :
...
theorem eval_X_pow (n : ?) : (X ^ n : R[X]).eval x = x ^ n := by
  simp
...
theorem eval?_finset_prod (s : Finset ?) (g : ? ? R[X]) (x : S) :
    (? i ? s, g i).eval? f x = ? i ? s, (g i).eval? f x :=
...
-/
theorem eval_prod {? : Type*} (s : Finset ?) (p : ? ? R[X]) (x : R) :
...
theorem prod_comp {? : Type*} (s : Finset ?) (p : ? ? R[X]) (q : R[X]) :
    (? j ? s, p j).comp q = ? j ? s, (p j).comp q :=
  map_prod (compRingHom q) _ _

theorem isRoot_prod {R} [CommSemiring R] [IsDomain R] {? : Type*} (s : Finset ?) (p : ? ? R[X])
...
protected theorem map_prod {? : Type*} (g : ? ? R[X]) (s : Finset ?) :
    (? i ? s, g i).map f = ? i ? s, (g i).map f :=
